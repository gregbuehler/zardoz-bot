...
module.exports = (robot) ->
  ...
  if process.env.HUBOT_ANNOUNCE_USERS
    allowedUsers = process.env.HUBOT_ANNOUNCE_USERS.split(',')
  else
    allowedUsers = []

  robot.respond /announce "(.*)"/i, (msg) ->
    announcement = msg.match[1]
    if msg.message.user.name in allowedUsers
      for room in allRooms
        robot.messageRoom room, announcement
    else
      msg.reply "Sorry, but you're not allowed to do that...#{msg.message.user.name}"
